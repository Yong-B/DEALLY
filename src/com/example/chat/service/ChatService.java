...
public class ChatService {
    ...
    @Autowired
    private ItemSelectOneUseCase itemSelectOneUseCase;
    ...
    public List<Map<String, Object>> getChatRoomsWithItemInfo(String userId) {
        ...
        for (ChatRoom room : chatRooms) {
            ...
            data.put("chatRoom", room);  // ChatRoom 객체 전체를 전달

            // 상품 정보 조회
            try {
                Item item = itemSelectOneUseCase.findById(room.getItemId());
                data.put("itemName", item.getItemName());
            } catch (Exception e) {
                data.put("itemName", "알 수 없는 상품");
                log.error("상품 정보 조회 실패: " + room.getItemId(), e);
            }
